...
import kotlinx.android.synthetic.main.activity_linear_vertical.*

class LinearVerticalActivity : AppCompatActivity(), JCellClickListener, JCellLongClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val recyclerView=activity_linear_vertical_recyclerview
        (1..20).map { Animal("Animal $it") }.forEach {
            recyclerView.addCell(AnimalCell(it).apply {
                setCellClickListener(this@LinearVerticalActivity)
            })
        ...
        (1..20).map { Movie("Animal $it",it.toFloat()) }.forEach {
            recyclerView.addCell(MovieCell(it).apply {
                setCellLongClickListener(this@LinearVerticalActivity)
            })
        }
    }
}
